...
USAGE:
    select_sequence needle, [haystack[, selName[, het[, matchMode]]]]
...
def select_sequence(needle, haystack="*", selName=None, het=0, matchMode="all"):
    """
    DESCRIPTION:
        Given a sequence/regex to find, select those matching amino acids in the
        protein.
    ...
    if not checkParams(needle, haystack, selName, het, matchMode):
        print(
            "There was an error with a parameter. Please see the above error message for how to fix it."
        )
        return
    ...
    for obj in cmd.get_object_list():
        cmd.iterate(
            f"%{obj} and {haystack_sel}",
            lambda atom: data[atom.model][(atom.segi, atom.chain)].append(
                (atom.resi, atom.oneletter)
            ),
        )
    ...
    print(
        f'select_sequence: selection "{selName}" defined with {cmd.count_atoms(selName)} atoms.'
    )
    return selName


cmd.extend("select_sequence", select_sequence)
cmd.select_sequence = select_sequence
cmd.auto_arg[1]["select_sequence"] = [cmd.object_sc, "object", ""]
cmd.auto_arg[2]["select_sequence"] = [
    lambda: cmd.Shortcut(["het=1", "matchMode=chain", "matchMode=first"]),
    "params",
    "",
]
cmd.auto_arg[3]["select_sequence"] = [
    lambda: cmd.Shortcut(["het=1", "matchMode=chain", "matchMode=first"]),
    "params",
    "",
]
...
def checkParams(needle, haystack, selName, het, matchMode):
    ...
    if not isinstance(haystack, str) or len(haystack) == 0:
        print(
            "Error: Please provide valid PyMOL object or selection name in which to search."
        )
        return False
...
def test_select_sequence():
    cmd.reinitialize()
    cmd.fab("ACDEFG", "m1")
    cmd.fab("HIKLMN", "m2")
    cmd.do("select_sequence DEF, *, foundSeq")
